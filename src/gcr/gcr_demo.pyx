...
from gcry_s_exp cimport gcry_sexp_t, gcry_sexp_sscan, gcry_sexp_length, gcry_sexp_nth_data, gcry_sexp_nth, gcry_sexp_car, gcry_sexp_cdr, gcry_sexp_find_token
    ...
    for i in range(lst_cnt):
        ...
        if data_ptr == NULL:
            ...
            for j in range(cnt_sub):
                data_ptr = gcry_sexp_nth_data(tmp_sub_exp_p, j, &data_len)
                print (f"Expression[{i}]  --=>      car.data[{j}]{{0-{cnt_sub - 1}}}:   ++> {cython.cast(str, data_ptr)}")

                tmp_seek_exp_p = gcry_sexp_find_token (tmp_exp_p, data_ptr, data_len)
                ...
                for k in range(cnt_seek):
                    data_ptr = gcry_sexp_nth_data(tmp_seek_exp_p, k, &data_len)
        ...
        tmp_seek_exp_p = gcry_sexp_find_token (s_exp_p, data_ptr, data_len)
        cnt_seek = gcry_sexp_length (tmp_seek_exp_p)
        print (f"Expression[{i}]  --=>      Token:<{cython.cast(str, data_ptr)}>:   ++> full searching count {cnt_seek}")
